...
class AddCredDialog(qt.Dialog):
    ...
    def _build_ui(self):
        ...
        layout.addRow(QtWidgets.QLabel(m.slot))
        self._slot = QtWidgets.QButtonGroup(self)
        slot1_status = m.in_use if self._slot_status[0] else m.free
        self._slot_1 = QtWidgets.QRadioButton(m.slot_2 % (1, slot1_status))
        ...
        self._slot_2 = QtWidgets.QRadioButton(m.slot_2 % (2, slot2_status))
        self._slot_2.setProperty('value', 2)
        self._slot.addButton(self._slot_2)
        layout.addRow(self._slot_2)

        self._touch = QtWidgets.QCheckBox(m.require_touch)
    ...
    def _save(self):
        if not self._cred_key.hasAcceptableInput():
            ...
            QtWidgets.QMessageBox.warning(self, m.no_slot, m.no_slot_desc)
        elif self._slot_status[self.slot - 1] \
            and QtWidgets.QMessageBox.Ok != QtWidgets.QMessageBox.warning(
    ...
    def _handle_qr(self, parsed):
        if parsed:
            if parsed['type'] != 'totp':
                QtWidgets.QMessageBox.warning(self, m.qr_not_supported,
